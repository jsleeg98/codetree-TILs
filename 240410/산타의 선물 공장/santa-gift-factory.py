...
def unload_box(query):
    ...
    for i in range(1, num_belt + 1):
        ...
        # 맨 뒤로 보내기
            ...
            belt_front_back[i][0] = box_list[front_id].back  # 해당 벨트 맨 앞 상자 id 변경
            box_list[box_list[front_id].back].front = -1  # 두번째 상자 front -1로 변경
...
box_list = [Box() for _ in range(100001)]  # 박스 객체 리스트, idx는 id
box_status = [0 for _ in range(100001)]  # 박스 상태 리스트, idx는 id
